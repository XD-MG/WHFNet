...
# for multiple images
@TRANSFORMS.register_module()
class LoadMultipleImageFromFile(LoadImageFromFile):
...
@TRANSFORMS.register_module()
class LoadMultipleImageFromArray(LoadImageFromFile):
    """
    from numpy array load opt-sar images
    for prediction
    """
    
    def __init__(self, to_float32=False, color_type='color', color_type2='color', **kwargs):
        super().__init__(**kwargs)
    ...
    def transform(self, results: Dict):
        ...
        assert img1 is not None, 'img1 array is None'
        assert img2 is not None, 'img2 array is None'
